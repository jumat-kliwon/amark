...
export default function CertificatePage() {
  ...
  const { certificates, pagination, links, isLoading } = useCertificates(currentPage);
  const { certificate: certificateDetail, isLoading: isLoadingDetail } = useCertificateDetail(selectedCertificateId);
  ...
  const shouldShowPagination = pagination && (links?.next || links?.prev || (pagination.current_page && pagination.current_page > 1));
  ...
  return (
    <div className="min-h-screen bg-background">
              ...
              {certificates.map((cert) => (
                ...
                >
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      <h3 className="mb-1 text-lg font-semibold">{cert.membership.name}</h3>
                  ...
                  <div className="flex items-center gap-3">
                    <Button variant="outline" size="sm" onClick={() => handlePreview(cert.id)}>
      ...
      <Dialog open={!!selectedCertificateId} onOpenChange={handleCloseModal}>
        <DialogContent className="max-w-3xl p-0 overflow-hidden">
          ...
          {isLoadingDetail ? (
            <div className="flex flex-col items-center justify-center p-12">
              <Loader2 className="mb-4 h-8 w-8 animate-spin text-muted-foreground" />
              <p className="text-sm text-muted-foreground">Memuat detail sertifikat...</p>
            </div>
          ) : certificateDetail ? (
            <div className="p-6">
              {/* Certificate Preview */}
              <div className="relative aspect-[1.414/1] w-full rounded-lg border-4 border-primary/20 bg-gradient-to-br from-card via-card to-muted p-8 shadow-inner">
                ...
                <div className="relative flex h-full flex-col items-center justify-center text-center">
                  ...
                  <p className="mb-6 text-2xl font-semibold">{certificateDetail.user.name}</p>
